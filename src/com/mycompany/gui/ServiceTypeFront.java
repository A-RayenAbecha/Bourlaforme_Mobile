...
public class ServiceTypeFront extends BaseForm{
     ...
     /*     Toolbar tb = new Toolbar(true);
        ...
        Button btnSeance = new Button("Reserver");
        addStringValue("", btnSeance);
        Button btnreserver = new Button("Annuler Réservation");
        addStringValue("", btnreserver);
...
////////////////////
 ...
 InfiniteProgress ip = new InfiniteProgress();; //Loading  after insert data
       final Dialog iDialog = ip.showInfiniteBlocking();
iDialog.dispose(); //na7io loading ba3d ma3mlna ajout
                    new     Crud_Commentaires_Front(res).show();
refreshTheme();//Actualisation
}
            catch(Exception ex ) {
                ex.printStackTrace();
...
btnreserver.addActionListener((e) -> {  //Appui sur boutton Service
            try {
 InfiniteProgress ip = new InfiniteProgress();; //Loading  after insert data
       final Dialog iDialog = ip.showInfiniteBlocking();
iDialog.dispose(); //na7io loading ba3d ma3mlna ajout
                    new     SuppReservationForm(res).show();
refreshTheme();//Actualisation
}
            catch(Exception ex ) {
                ex.printStackTrace();
