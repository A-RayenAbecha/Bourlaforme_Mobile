 ...
 */
public class Crud_Seance extends BaseForm{
    
        Form current;
         
          public Crud_Seance(Resources res)
          {
...
////////////////////
  ...
  btnAffiche.addActionListener((e) -> {  //Appui sur boutton article
            try {
 InfiniteProgress ip = new InfiniteProgress();; //Loading  after insert data
       final Dialog iDialog = ip.showInfiniteBlocking();
iDialog.dispose(); //na7io loading ba3d ma3mlna ajout
                    new     ListSeanceForm(res).show();
refreshTheme();//Actualisation
}
            catch(Exception ex ) {
                ex.printStackTrace();
