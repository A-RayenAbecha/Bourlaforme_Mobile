 ...
 */
package com.mycompany.gui.front.panier;

import com.mycompany.gui.front.AccueilFront;
import com.mycompany.services.CheckoutService;
...
public class Payment extends Form {
    ...
    private void addActions() {
        ...
        manageButton.addActionListener(action -> {
            ...
            String message = PanierService.getInstance().Payer(numCard, expMois, exAnnee, cvv, (int) prix);
            if (message.equals("true")) {
                Dialog.show("Success", "payment success", new Command("OK"));
                int responseCode = CheckoutService.getInstance().chekout();
                if (responseCode == 200) {
    ...
    private void showBackAndRefresh() {
        previous.showBack();
        new com.mycompany.gui.front.panier.ShowAll(new AccueilFront()).show();
